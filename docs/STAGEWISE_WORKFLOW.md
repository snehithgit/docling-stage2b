...
6. **Stage 3** applies accepted overlays in memory and calls the configured existing Docling Serve HybridChunker. A deterministic post-validator preserves logical boundaries while reducing formatting-driven oversize. Raw Docling is never rewritten.
7. **Retrieval quality** builds a local searchable index from eligible Stage 3 chunks, excludes formatting/repetition noise, and measures Top-1/3/5 retrieval. Search itself is deterministic and makes no LLM call.
8. **Answer generation (optional)** sends only the retrieved evidence to the explicitly selected Pi5, OnePlus, or Groq generator. No automatic fallback is allowed. The same evidence can be copied as a portable `[S#]`-cited prompt without calling any model.
...
- **RAG quality:** deterministic local search, retrieval audit, fixed benchmark cases, optional grounded answer generation, and portable evidence export for external LLMs.
