...
- Each Stage 2A run writes a separate `__runN` result directory. The database points to it only after all reports and empty current overlays are ready. Previous directories remain available as historical artifacts.
- Reruns carry prior ledger entries forward as superseded and use a new verification generation, even when routes are unchanged. Late verifier results cannot write into the new run.
