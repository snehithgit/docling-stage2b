...
def test_old_partial_source_transcription_is_demoted_and_overlay_removed(tmp_path: Path):
    ...
    assert saved["status_reason"].endswith("_KEEP_ORIGINAL")
    assert saved["safety_revalidation"]["reason"] in {"CRITICAL_SOURCE_TOKEN_NOT_PRESERVED", "SOURCE_CONTENT_CONTRACTION", "INCOMPLETE_SOURCE_TRANSCRIPTION"}
...
def test_stage2c_safety_profile_rejects_table_cell_neighbor_contamination():
    original = "The purge / sampling indicator is flashing in red."
    proposed = "The ball in the flow meter is stuck The purge / sampling indicator is flashing in red. Operate the pump on again Connect"
    profile = source_transcription_safety_profile(original, proposed, source_type="table_cell")
    assert profile["accepted"] is False
    assert "TABLE_CELL_CONTEXT_CONTAMINATION" in profile["reasons"]


def test_stage2c_upsert_demotes_old_scope_inversion(tmp_path: Path):
    result_dir = tmp_path / "scope-inversion"
    original = "document must not be copied without d d a ty unauthorized purpose."
    proposed = "Contrainvention will be prosecuted."
    removed = "This document must not be copied without our written permission and the contents thereof must not be imparted to a third party nor be used for any unauthorized purpose."
    upsert_ledger_entry(result_dir, "abc", {
        "entry_id": "g:text:R83", "entry_type": "text_correction", "route_id": "R00083",
        "status": "applied", "status_reason": "SOURCE_IMAGE_TARGET_RECONSTRUCTION",
        "original_text": original, "proposed_text": proposed, "human_verified": False,
        "source_type": "text", "scope_guard": {"removed_prefix": removed, "removed_suffix": "", "reasons": ["TARGET_SCOPE_CONFIRMED"]},
    })
    ledger = json.loads((result_dir / "correction_ledger.json").read_text())
    entry = ledger["entries"][0]
    assert entry["status"] == "pending"
    assert entry["proposed_text"] is None
    assert "SCOPE_INVERSION_DISCARDED_TARGET" in entry["status_reason"]


def test_stage2c_upsert_demotes_table_cell_contamination(tmp_path: Path):
    result_dir = tmp_path / "table-contamination"
    upsert_ledger_entry(result_dir, "abc", {
        "entry_id": "g:table-cell:R136", "entry_type": "table_cell_correction", "route_id": "R00136",
        "status": "applied", "status_reason": "SOURCE_IMAGE_TARGET_RECONSTRUCTION",
        "original_text": "The purge / sampling indicator is flashing in red.",
        "proposed_text": "The ball in the flow meter is stuck The purge / sampling indicator is flashing in red. Operate the pump on again Connect",
        "human_verified": False, "source_type": "table_cell", "table_index": 1, "cell_index": 2,
    })
    ledger = json.loads((result_dir / "correction_ledger.json").read_text())
    entry = ledger["entries"][0]
    assert entry["status"] == "pending"
    assert "TABLE_CELL_CONTEXT_CONTAMINATION" in entry["status_reason"]


def test_technical_prose_detects_fault_and_imperative_without_numbers():
    profile = technical_format_profile("If the pump fails to prime, check the suction valve and inspect the suction line for air leakage.")
    assert profile["is_technical_format"] is False
    assert profile["is_technical_prose"] is True
    assert profile["is_technical"] is True
    assert "conditional_fault" in profile["prose_classes"]
    assert "imperative_check" in profile["prose_classes"]


def test_source_transcription_rejects_dropped_troubleshooting_action_clause():
    original = "Short circuit. Check output cable is not connected to 0V (ground). Replace the MC card."
    proposed = "Short circuit. Check output cable is not connected to 0V (ground)."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["accepted"] is False
    assert "TROUBLESHOOTING_ACTION_DROPPED" in profile["reasons"]
    assert "replace" in profile["missing_action_verbs"]


def test_source_transcription_rejects_dropped_repeated_verb_obligation():
    original = "Replace the MC card. Replace the damaged cable."
    proposed = "Replace the damaged cable."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["accepted"] is False
    assert "TROUBLESHOOTING_ACTION_DROPPED" in profile["reasons"]
    assert "replace" in profile["missing_action_verbs"]
    assert "replace mc card" in profile["missing_action_obligations"]


def test_source_transcription_action_obligations_allow_normalized_reordering():
    original = "Replace the MC card. Check the connector."
    proposed = "CHECK connector; replace a MC card."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["missing_action_obligations"] == []
    assert "TROUBLESHOOTING_ACTION_DROPPED" not in profile["reasons"]
    assert profile["accepted"] is True


def test_source_transcription_compound_object_is_one_obligation():
    text = "Replace the fuse and holder assembly."
    profile = source_transcription_safety_profile(text, text, source_type="text")
    assert profile["accepted"] is True
    assert profile["missing_action_obligations"] == []


def test_source_transcription_and_second_action_detects_dropped_obligation():
    original = "Replace the MC card and check the connector."
    proposed = "Replace the MC card."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["accepted"] is False
    assert "TROUBLESHOOTING_ACTION_DROPPED" in profile["reasons"]
    assert "check connector" in profile["missing_action_obligations"]


def test_source_transcription_repeated_identical_obligation_is_counted():
    original = "Check the oil level. Check the oil level."
    proposed = "Check the oil level."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["accepted"] is False
    assert profile["missing_action_obligations"].count("check oil level") == 1


def test_source_transcription_action_object_tolerates_ocr_spelling_cleanup():
    original = "Replace teh MC card."
    proposed = "Replace the MC card."
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["missing_action_obligations"] == []
    assert "TROUBLESHOOTING_ACTION_DROPPED" not in profile["reasons"]
...
def test_source_transcription_rejects_dropped_formula_result():
    original = "Pulse count = 700 x 190 = 133000 pulses"
    proposed = "Pulse count = 700 x 190"
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["accepted"] is False
    assert "CRITICAL_SOURCE_TOKEN_NOT_PRESERVED" in profile["reasons"]


def test_source_transcription_allows_format_only_critical_token_spacing_change():
    original = "Supply voltage 24 V"
    proposed = "Supply voltage 24V"
    profile = source_transcription_safety_profile(original, proposed, source_type="text")
    assert profile["critical_tokens_changed"] is False
    assert profile["accepted"] is True


def test_legacy_applied_action_dropping_correction_is_demoted_without_model_rerun(tmp_path: Path):
    result_dir = tmp_path / "run"
    result_dir.mkdir()
    entry = {
        "entry_id": "old:text:R1", "entry_type": "text_correction", "route_id": "R1",
        "status": "applied", "status_reason": "SOURCE_IMAGE_TARGET_RECONSTRUCTION",
        "original_text": "Fault. Check cable. Replace the MC card.",
        "proposed_text": "Fault. Check cable.", "human_verified": False,
    }
    (result_dir / "correction_ledger.json").write_text(json.dumps({"entries": [entry]}), encoding="utf-8")
    assert normalize_human_verified_ledger(result_dir) == 1
    ...
    assert saved["status"] == "pending"
    assert saved["proposed_text"] is None
    assert saved["safety_revalidation"]["model_rerun"] is False
    assert saved["safety_revalidation"]["reason"] == "TROUBLESHOOTING_ACTION_DROPPED"


def test_overlap_arbitration_keeps_applied_sweep_over_pending_normal(tmp_path: Path):
    result_dir = tmp_path / "book"
    result_dir.mkdir()
    sweep = {
        "entry_id": "g:vision:AV000001", "entry_type": "vision_enrichment",
        "route_id": "AV000001", "generation": "g", "source_index": 1,
        "artifact_sweep": True, "status": "applied", "status_reason": "TECHNICAL_VISUAL_ENRICHMENT",
        "verification_verdict": "TECHNICAL_USEFUL", "generated_summary": "useful sweep evidence",
    }
    normal = {
        "entry_id": "g:vision:V1", "entry_type": "vision_enrichment",
        "route_id": "V1", "generation": "g", "source_index": 1,
        "artifact_sweep": False, "status": "pending", "status_reason": "VISION_UNCERTAIN",
        "verification_verdict": "UNCERTAIN", "generated_summary": "uncertain normal evidence",
    }
    upsert_ledger_entry(result_dir, "sha", sweep)
    upsert_ledger_entry(result_dir, "sha", normal)
    ledger = json.loads((result_dir / "correction_ledger.json").read_text())
    by_route = {e["route_id"]: e for e in ledger["entries"]}
    assert by_route["AV000001"]["status"] == "applied"
    assert by_route["V1"]["status"] == "superseded"
    overlays = (result_dir / "chunk_overlays.jsonl").read_text()
    assert "useful sweep evidence" in overlays
    assert "uncertain normal evidence" not in overlays


def test_overlap_arbitration_prefers_normal_when_both_applied(tmp_path: Path):
    result_dir = tmp_path / "book"
    result_dir.mkdir()
    sweep = {
        "entry_id": "g:vision:AV000002", "entry_type": "vision_enrichment",
        "route_id": "AV000002", "generation": "g", "source_index": 2,
        "artifact_sweep": True, "status": "applied", "status_reason": "TECHNICAL_VISUAL_ENRICHMENT",
        "verification_verdict": "TECHNICAL_USEFUL", "generated_summary": "sweep",
    }
    normal = {
        "entry_id": "g:vision:V2", "entry_type": "vision_enrichment",
        "route_id": "V2", "generation": "g", "source_index": 2,
        "artifact_sweep": False, "status": "applied", "status_reason": "TECHNICAL_VISUAL_ENRICHMENT",
        "verification_verdict": "TECHNICAL_USEFUL", "generated_summary": "normal",
    }
    upsert_ledger_entry(result_dir, "sha", sweep)
    upsert_ledger_entry(result_dir, "sha", normal)
    ledger = json.loads((result_dir / "correction_ledger.json").read_text())
    by_route = {e["route_id"]: e for e in ledger["entries"]}
    assert by_route["V2"]["status"] == "applied"
    assert by_route["AV000002"]["status"] == "superseded"


def test_overlap_arbitration_restores_legacy_applied_sweep_superseded_by_4062(tmp_path: Path):
    result_dir = tmp_path / "book"
    result_dir.mkdir()
    ledger = {
        "entries": [
            {
                "entry_id": "g:vision:AV000003", "entry_type": "vision_enrichment",
                "route_id": "AV000003", "generation": "g", "source_index": 3,
                "status": "superseded", "status_reason": "SUPERSEDED_BY_NORMAL_PICTURE_ROUTE",
                "verification_verdict": "TECHNICAL_USEFUL", "generated_summary": "legacy sweep",
            },
            {
                "entry_id": "g:vision:V3", "entry_type": "vision_enrichment",
                "route_id": "V3", "generation": "g", "source_index": 3,
                "status": "excluded", "status_reason": "DECORATIVE_OR_LOW_VALUE",
                "verification_verdict": "DECORATIVE_OR_LOW_VALUE",
            },
        ]
    }
    (result_dir / "correction_ledger.json").write_text(json.dumps(ledger))
    from app.stage2c import supersede_vision_entries_for_routes
    assert supersede_vision_entries_for_routes(result_dir, {"AV000003"}) >= 1
    updated = json.loads((result_dir / "correction_ledger.json").read_text())
