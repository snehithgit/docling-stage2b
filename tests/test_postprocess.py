...
def test_table_cell_route_preserves_structural_span_metadata(tmp_path):
    doc = synthetic_doc()
    doc["tables"] = [{
        "prov": [{"page_no": 1}],
        "data": {"table_cells": [{
    ...
    table_routes = [route for route in routes["routes"] if (route.get("source") or {}).get("type") == "table_cell"]
    assert table_routes
    source = table_routes[0]["source"]
    assert source["table_index"] == 0
    assert source["cell_index"] == 0
    assert source["row_start"] == 5
    assert source["row_end"] == 8
    assert source["col_start"] == 2
    assert source["col_end"] == 4
...
def test_stage2_rerun_reuses_existing_converted_zip_without_database_reset(tmp_path):
    async def run():
        ...
        upsert_ledger_entry(processed_dir / result_dir, "abc", {
            "entry_id": "human:text:R2", "entry_type": "text_correction",
            "route_id": "R2", "status": "applied", "proposed_text": "human correction",
            "human_verified": True,
        })
        ...
        by_id = {entry["entry_id"]: entry for entry in ledger["entries"]}
        assert by_id["old:text:R1"]["status"] == "superseded"
        assert by_id["old:text:R1"]["status_reason"] == "STAGE2A_GENERATION_RERUN"
        assert by_id["human:text:R2"]["status"] == "applied"
        assert by_id["human:text:R2"]["human_verified"] is True
        assert ledger["rule_version"] == "stage2c-source-fidelity-v8"
        assert "old correction" in (processed_dir / result_dir / "chunk_overlays.jsonl").read_text()
        new_overlays = (processed_dir / second["result_dir"] / "chunk_overlays.jsonl").read_text()
        assert "old correction" not in new_overlays
        assert "human correction" in new_overlays
...
def test_routes_have_deterministic_review_priority_score_and_page_ocr_risk(tmp_path):
    doc = _ocr_recall_doc(["� � � broken OCR", "hydraulic pressure 16 bar", "hydaulic pressure 16 bar"])
    cfg = AppConfig(database_path=str(tmp_path / "jobs.db"), stage2a_ocr_recall_rare_max_count=20)
    diagnostics = build_diagnostics(doc, cfg)
    routes = build_routes(doc, diagnostics, cfg)["routes"]
    assert routes
    assert all(0 <= int(route.get("review_priority_score") or 0) <= 100 for route in routes)
    text_routes = [route for route in routes if route.get("target") == "pi5"]
    if text_routes:
        assert "ocr_risk_score" in (text_routes[0].get("source") or {})


def test_vision_review_priority_prefers_technical_diagram_over_decorative():
    cfg = AppConfig(database_path=":memory:")
    diagnostics = {"signals": [{
        "classification": "VISION_REVIEW", "code": "LOW_CONFIDENCE_VISUAL", "items": [
            {"picture_index": 1, "page": 1, "class": "engineering_drawing", "confidence": 0.3, "reason": "ambiguous"},
            {"picture_index": 2, "page": 1, "class": "decorative_photo", "confidence": 0.3, "reason": "ambiguous"},
        ]
    }]}
