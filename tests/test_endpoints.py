...
class EndpointTests(unittest.TestCase):
    ...
    def test_home_and_version_identify_guided_workspace(self):
        ...
        with TestClient(self.main.app) as client:
            page = client.get("/")
            self.assertIn("<h1>Books</h1>", page.text)
...
class VerificationEndpointTests(unittest.TestCase):
    ...
    def test_vision_audit_page_and_read_only_audit_endpoint(self):
        ...
        with TestClient(self.main.app) as client:
            ...
            self.assertEqual(page.status_code, 200)
            self.assertIn("Verifier audit", page.text)
    ...
    def test_text_audit_page_and_read_only_audit_endpoint(self):
        ...
        asyncio.run(self.main.runtime.stage2b_store.sync_routes(9012, 9012, "gt", routes, "book__job9012", "audit-text.zip"))
        rows = asyncio.run(self.main.runtime.stage2b_store.list_book_jobs_raw(9012))
        job_id = rows[0]["id"]
        asyncio.run(self.main.runtime.stage2b_store.mark_completed(
            job_id, 2.0, "text-model", "http://text", "LIKELY_CORRUPT",
            {"task": "source_image_target_reconstruction", "page": 8, "text_index": 7, "source_type": "text", "suspect_text": "Transmilter Terminal", "before_anchors": ["Above"], "after_anchors": ["Below"], "selected_processor": "pi5", "target_crop": {"mode": "docling_target_bbox", "source_kind": "pdf", "page": 8, "clip_points": [1,2,3,4], "scale": 2.5}},
        ...
        with TestClient(self.main.app) as client:
            ...
            audit = client.get("/api/stage2b/text-audit?postprocess_job_id=9012")
            self.assertEqual(audit.status_code, 200)
            body = audit.json()
            self.assertEqual(body["summary"]["total"], 1)
            self.assertEqual(body["summary"]["applied"], 1)
    ...
    def test_retry_all_failed_requeues_text_and_vision_only(self):
        routes = [
            {"route_id": "RF1", "target": "pi5", "code": "OCR_GARBLE", "priority": "medium", "source": {"type": "text", "index": 1, "page": 2}},
            {"route_id": "RF2", "target": "oneplus", "code": "VISUAL", "priority": "medium", "source": {"type": "picture", "index": 2, "page": 3}},
            {"route_id": "RF3", "target": "pi5", "code": "OCR_GARBLE", "priority": "low", "source": {"type": "text", "index": 3, "page": 4}},
        ]
        asyncio.run(self.main.runtime.stage2b_store.sync_routes(9020, 9020, "gr", routes, "book__job9020", "retry.zip"))
        rows = asyncio.run(self.main.runtime.stage2b_store.list_book_jobs_raw(9020))
        by_route = {row["route_id"]: row for row in rows}
        asyncio.run(self.main.runtime.stage2b_store.mark_failed(by_route["RF1"]["id"], "TestError", "text"))
        asyncio.run(self.main.runtime.stage2b_store.mark_failed(by_route["RF2"]["id"], "TestError", "vision"))
        with TestClient(self.main.app) as client:
            response = client.post("/api/stage2b/retry-all-failed")
            self.assertEqual(response.status_code, 200)
            body = response.json()
            self.assertTrue(body["accepted"])
            self.assertEqual(body["retried"]["pi5"], 1)
            self.assertEqual(body["retried"]["oneplus"], 1)
            self.assertTrue(body["successful_jobs_untouched"])
            self.assertTrue(body["pending_jobs_untouched"])
        current = {row["route_id"]: row for row in asyncio.run(self.main.runtime.stage2b_store.list_book_jobs_raw(9020))}
        self.assertEqual(current["RF1"]["status"], "pending")
        self.assertEqual(current["RF2"]["status"], "pending")
        self.assertEqual(current["RF1"]["authorized"], 1)
        self.assertEqual(current["RF2"]["authorized"], 1)
        self.assertEqual(current["RF3"]["authorized"], 0)
