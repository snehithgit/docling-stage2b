...
def test_delete_equipment_removes_derived_machine_embedding_index(tmp_path: Path):
    books = [{"postprocess_job_id": 1, "source_filename": "manual.pdf", "result_dir": "book", "index_ready": True}]
    saved = upsert_equipment(tmp_path, books, name="Deck Crane", manuals=[{"postprocess_job_id": 1, "manual_type": "operation"}])
