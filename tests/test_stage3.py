...
async def test_stage3_uses_remote_docling_ip_and_applies_overlays_in_memory(tmp_path: Path):
    ...
    assert rows[0]["retrieval"]["eligible"] is True
...
"""
    ...
    # .30 first removes Markdown padding/separator cost. If that alone brings
    # the table under budget, keeping one coherent table is safer than splitting.
    assert len(rows) == 1
    assert rows[0]["num_tokens"] <= 256
    assert rows[0]["num_tokens_estimated"] is True
    assert rows[0]["stage3_postprocess"]["action"] == "compact_oversized_markdown_text"
    assert rows[0]["raw_text"] == raw
    assert "|   NO. | APPELLATION" in rows[0]["raw_text"]
    for number in (1, 2, 3, 4):
        assert rows[0]["raw_text"].count(f"|     {number} |") == 1
...
async def test_stage3_does_not_treat_stage2a_empty_scaffold_as_finalized_stage2c(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="Stage 2C"):
...
async def test_stage3_blocks_until_likely_corrupt_text_has_human_decision(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="Verifier Audit is still required"):
        await builder.start(1)
...
def test_stage3_compacts_markdown_padding_without_changing_raw_table():
    raw = """| Possible Cause                                             | Recommended Action                                                                                                                                                              |
|------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Low oil                                                    | Fill oil                                                                                                                                                                        |
"""
    chunk = {
        ...
        "raw_text": raw,
        "num_tokens": 280,
        "headings": ["Chapter 2", "2.3 Troubleshooting"],
    }
    compacted = Stage3ChunkBuilder._compact_oversized_markdown_text(chunk, 256)
    assert compacted is not None
    assert compacted["raw_text"] == raw
    assert "---" not in compacted["text"]
    assert "Possible Cause" in compacted["text"]
    assert compacted["num_tokens"] < 280
    assert compacted["stage3_postprocess"]["raw_text_unchanged"] is True


def test_stage3_splits_oversized_prose_only_at_existing_sentence_boundaries():
    raw = ("First complete sentence with useful technical context. " * 6).strip() + "\n\n" + ("Second complete sentence with another diagnostic action. " * 6).strip()
    chunk = {"text": "Troubleshooting\n" + raw, "raw_text": raw, "num_tokens": 400, "headings": ["Troubleshooting"]}
    children = Stage3ChunkBuilder._split_oversized_logical_blocks(chunk, 256)
    assert len(children) >= 2
    assert all(child["stage3_postprocess"]["blind_character_split"] is False for child in children)
    assert "First complete sentence" in children[0]["raw_text"]


def test_stage3_recursively_compacts_oversized_header_child_from_previous_release():
    raw = "| Trouble                                                           | Probable Causes                                       | Remedies                                                                                                              |\n|-------------------------------------------------------------------|-------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------|\n"
    chunk = {
        "text": "Chapter 14\n" + raw, "raw_text": raw, "num_tokens": 265,
        "num_tokens_estimated": True, "headings": ["Chapter 14"],
        "stage3_postprocess": {"action": "split_oversized_logical_blocks"},
    }
    rows, stats = Stage3ChunkBuilder._post_validate_chunks(
        [chunk], max_tokens=256, enforce=True, repeat_table_header=True
    )
    assert stats["oversized_chunks_remaining"] == 0
    assert rows[0]["num_tokens"] <= 256
    assert rows[0]["raw_text"] == raw


def test_stage3_compacts_separator_only_chunk_instead_of_retaining_noise():
    raw = "|" + "-" * 250 + "|\n"
    chunk = {"text": "Chapter 3\n" + raw, "raw_text": raw, "num_tokens": 263, "headings": ["Chapter 3"]}
    rows, stats = Stage3ChunkBuilder._post_validate_chunks(
        [chunk], max_tokens=256, enforce=True, repeat_table_header=True
    )
    assert stats["oversized_chunks_remaining"] == 0
    assert rows[0]["raw_text"] == raw
    assert rows[0]["text"] == "Chapter 3\n"
    assert rows[0]["stage3_postprocess"]["action"] == "compact_separator_only_markup"


def test_stage3_retrieval_artifacts_write_table_evidence_file(tmp_path):
    from app.retrieval import write_retrieval_artifacts
    result_dir = tmp_path / "Manual__job6__run1"
    result_dir.mkdir()
    (result_dir / "source_manifest.json").write_text('{"source_filename":"Manual.pdf"}', encoding="utf-8")
    chunks = [
        {"chunk_id":"CHK-000001","chunk_index":0,"text":"BASIC DATA\n| Capacity (ton) | Speed (m/min) |","raw_text":"| Capacity (ton) | Speed (m/min) |","num_tokens":8,"headings":["BASIC DATA"],"doc_items":["#/tables/11"],"page_numbers":[57]},
        {"chunk_id":"CHK-000002","chunk_index":1,"text":"| |\n|---|---|","raw_text":"| |\n|---|---|","num_tokens":3,"headings":["BASIC DATA"],"doc_items":["#/tables/11"],"page_numbers":[57]},
        {"chunk_id":"CHK-000003","chunk_index":2,"text":"| 36 | 23 |","raw_text":"| 36 | 23 |","num_tokens":4,"headings":["BASIC DATA"],"doc_items":["#/tables/11"],"page_numbers":[57]},
    ]
    summary = write_retrieval_artifacts(result_dir, chunks, max_tokens=256)
