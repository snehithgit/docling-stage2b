...
def test_phone_script_contains_proven_launch_and_phone_side_lifecycle():
    ...
    for token in ["LLAMA_CPUS=\"4,5,6,7\"", "LLAMA_NICE=10", "taskset -c $LLAMA_CPUS", "nice -n $LLAMA_NICE", "-t 4", "-tb 4", "-c 4096", "-np 1", "--reasoning off", "--reasoning-budget 0", "--image-max-tokens 1024", "--host 0.0.0.0", "--port 8080"]:
