    ...
    async def _process_one(self, authorized_only: bool = False) -> bool:
        ...
        try:
            ...
            while True:
                ...
                if (
                    not long_running_notified
                    and elapsed > config.document_timeout_minutes * 60
                ):
                    # A healthy remote Docling task can legitimately run for
                    # several hours on OCR-heavy manuals. Keep the saved task_id
                    # attached and continue polling; this threshold is only a UI
                    # warning, never a reason to submit the document twice.
                    long_running_notified = True
                    self._events.notify("processing_long_running")
                try:
                ...
                if task_status == "failure":
                    # This is a real terminal Docling failure, unlike a local
                    # timeout/poll/result-transfer problem. Clear the remote ID
                    # so an explicit Retry submits a fresh conversion.
