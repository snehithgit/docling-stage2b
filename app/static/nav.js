...
(() => {
  const version = "2026.09.22.40.10.1";
...
(() => {
  ...
  if (nav) {
    ...
    if (rag) {
      ...
      if (label) {
        ...
        label.appendChild(document.createTextNode('Machine RAG'));
      }
  ...
  const guides = {
    ...
    '/convert': ['Manual conversion', 'Use this only for one-off conversion. A converted document still enters the same ordered analysis → verification → finalization → chunking → machine embedding pipeline before RAG.', 'Convert one document'],
    '/queue': ['Conversion & settings', 'Add documents here. Conversion and Stage 2A analysis happen before verification; changing upstream data invalidates downstream stages.', 'Add or inspect a conversion'],
    ...
    '/retrieval': ['Machine-scoped retrieval', 'Normal Hybrid RAG is one physical machine at a time. All assigned manuals share one machine embedding index; unrelated machines are never searched together.', 'Select a machine'],
