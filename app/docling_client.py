...
class DoclingClient:
    ...
    @staticmethod
    def _result_timeout(config: AppConfig) -> httpx.Timeout:
        return httpx.Timeout(connect=10.0, read=float(config.docling_result_timeout_seconds), write=60.0, pool=10.0)
    ...
    async def submit_hybrid_chunks_json(
        self,
        *,
        filename: str,
        content: bytes,
        ...
        """Submit an existing Docling JSON document to remote HybridChunker.

        This uses the project's configured ``docling_url`` and the dedicated
        chunk endpoint. Feeding ``json_docling`` avoids re-running PDF parsing or
        OCR while still letting the Docling service own HybridChunker/tokenizer
        dependencies.
        """
        config: AppConfig = self._config_getter()
        fields: dict[str, Any] = {
            ...
            "convert_abort_on_error": "true",
            "chunking_max_tokens": str(int(max_tokens)),
            "chunking_tokenizer": tokenizer,
            "chunking_merge_peers": str(bool(merge_peers)).lower(),
            "chunking_use_markdown_tables": str(bool(use_markdown_tables)).lower(),
            "chunking_include_raw_text": str(bool(include_raw_text)).lower(),
            "chunking_use_markdown_images": "false",
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self._timeout()) as client:
                response = await client.post(
                    f"{config.docling_url}/v1/chunk/hybrid/file/async",
                    data=fields,
                    files=files,
                )
            self._raise_for_response(response, "submit HybridChunker job")
            task_id = response.json().get("task_id")
            if not task_id:
                raise DoclingApiError("Docling Serve returned no task_id for HybridChunker.")
            return str(task_id)
        except (OSError, httpx.HTTPError, ValueError) as exc:
            raise DoclingApiError(f"Unable to submit HybridChunker job: {exc}") from exc
    ...
    async def result(self, task_id: str) -> ResultPayload:
        config: AppConfig = self._config_getter()
        try:
            async with httpx.AsyncClient(timeout=self._result_timeout(config)) as client:
