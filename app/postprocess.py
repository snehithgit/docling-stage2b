...
def _docling_integrity_findings(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    for ti,table in enumerate(tables):
        ...
        for ci,cell in enumerate(cells):
            ...
            ua=_unicode_anomalies(value)
            if ua:
                findings["unicode"].append({
                    "source_type": "table_cell", "table_index": ti, "cell_index": ci, "page": page,
                    "row_start": cell.get("start_row_offset_idx"), "row_end": cell.get("end_row_offset_idx"),
                    "col_start": cell.get("start_col_offset_idx"), "col_end": cell.get("end_col_offset_idx"),
                    "text": value[:240], "anomalies": ua[:12],
                })
...
def _table_cell_ocr_candidates(doc: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for ti,table in enumerate(doc.get("tables") or []):
        ...
        for ci,cell in enumerate(((table.get("data") or {}).get("table_cells") or [])):
            ...
            if reasons:
                out.append({
                    "table_index": ti, "cell_index": ci, "page": page,
                    ...
                    "reasons": sorted(set(reasons)), "text": value[:300],
                    "contains_technical_value": bool(TECH_VALUE_RE.search(value) or INEQUALITY_RE.search(value)),
                })
    return out
...
def _page_ocr_risk_scores(doc: dict[str, Any]) -> dict[int, dict[str, Any]]:
    """Deterministic OCR-risk estimate, not a claimed OCR confidence value."""
    buckets: dict[int, list[str]] = defaultdict(list)
    for item in doc.get("texts") or []:
        page = _page_of(item)
        if page is not None:
            buckets[int(page)].append(str(item.get("text") or ""))
    output: dict[int, dict[str, Any]] = {}
    for page, values in buckets.items():
        text = "\n".join(values)
        chars = max(1, len(text))
        replacement = sum(text.count(ch) for ch in ("�", "□", "¤")) / chars
        punct_runs = len(re.findall(r"[^\w\s]{4,}", text)) / max(1, len(values))
        tiny_fragments = sum(1 for value in values if 0 < len(re.sub(r"\s+", "", value)) <= 3) / max(1, len(values))
        spaced_chars = len(re.findall(r"(?:\b[A-Za-z0-9]\s+){3,}[A-Za-z0-9]\b", text)) / max(1, len(values))
        risk = min(100.0, replacement * 900 + punct_runs * 18 + tiny_fragments * 35 + spaced_chars * 25)
        reasons = []
        if replacement > 0: reasons.append("replacement_characters")
        if punct_runs > 0.08: reasons.append("punctuation_runs")
        if tiny_fragments > 0.15: reasons.append("tiny_fragments")
        if spaced_chars > 0.05: reasons.append("spaced_character_runs")
        output[page] = {"ocr_risk_score": int(round(risk)), "reasons": reasons}
    return output
...
def build_profile(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "page_ocr_risk": {str(page): value for page, value in sorted(_page_ocr_risk_scores(doc).items())},
...
def build_diagnostics(doc: dict[str, Any], config: AppConfig, integrity: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    pages = doc.get("pages") or {}
    page_count = len(pages)
    page_ocr_risk = _page_ocr_risk_scores(doc)
    ...
    for item in table_recall_all:
        ...
        mapped=dict(item); mapped.pop("text_index",None)
        cell = ((tables[ti].get("data") or {}).get("table_cells") or [])[ci]
        mapped.update({
            "table_index": ti, "cell_index": ci, "page": page,
            "row_start": cell.get("start_row_offset_idx"),
            "row_end": cell.get("end_row_offset_idx"),
    ...
    return {
        ...
        "page_ocr_risk": {str(page): value for page, value in sorted(page_ocr_risk.items())},
        "signals": signals,
    }
...
def _review_priority_score(*, target: str, code: str, priority: str, source: dict[str, Any], reason: str) -> int:
    """Deterministic 0-100 review urgency inside the existing priority bands.

    This never changes verifier verdicts.  It only orders scarce/manual review
    work so technical/safety evidence is handled before low-value ambiguity.
    """
    base = {"high": 80, "medium": 55, "low": 30, "info": 10}.get(str(priority), 25)
    score = float(base)
    code_u = str(code or "").upper()
    reason_l = str(reason or "").lower()
    source_type = str(source.get("type") or "")
    try:
        score += min(12.0, max(0.0, float(source.get("ocr_risk_score") or 0)) * 0.12)
    except (TypeError, ValueError):
        pass
    if source.get("ocr_recall_score") is not None:
        try:
            score += min(10.0, max(0.0, float(source["ocr_recall_score"]) * 10.0))
        except (TypeError, ValueError):
            pass
    if any(term in code_u for term in ("TABLE", "OCR", "TECHNICAL", "SOURCE_PDF")):
        score += 5
    if any(term in reason_l for term in ("safety", "warning", "alarm", "fault", "trip", "pressure", "voltage", "current", "temperature", "torque")):
        score += 8
    if source_type == "table_cell":
        score += 8
    if target == "oneplus":
        category = str(source.get("class") or "").lower()
        technical = {
            "engineering_drawing", "flow_chart", "screenshot_from_manual", "table",
            "line_chart", "bar_chart", "box_plot", "full_page_image", "geographical_map",
            "electrical_schematic", "wiring_diagram", "single_line_diagram", "ladder_logic",
            "hydraulic_schematic", "pneumatic_schematic", "pid", "control_logic",
            "terminal_diagram", "exploded_view", "nameplate_or_data_plate", "table_or_schedule",
        }
        decorative = {"decorative_photo", "logo", "signature_or_stamp", "cover_art"}
        if category in technical:
            score += 15
        elif category in decorative:
            score -= 20
        try:
        ...
        if confidence is None:
            ...
            score += 6
        elif confidence >= 0.8 and category in decorative:
            score -= 5
...
def build_routes(doc: dict[str, Any], diagnostics: dict[str, Any], config: AppConfig) -> dict[str, Any]:
    ...
    def with_page_risk(source: dict[str, Any]) -> dict[str, Any]:
        ...
        risk = page_ocr_risk.get(str(page)) if page is not None else None
        if isinstance(risk, dict):
            source["ocr_risk_score"] = int(risk.get("ocr_risk_score") or 0)
            source["ocr_risk_reasons"] = list(risk.get("reasons") or [])
        return source
    ...
    def add(target: str, code: str, priority: str, source: dict[str, Any], action: str, reason: str) -> None:
        ...
        route = {
            ...
            "code": code,
            "priority": priority,
            "review_priority_score": _review_priority_score(target=target, code=code, priority=priority, source=source, reason=reason),
            "status": "pending",
            "source": source,
            "action": action,
            "reason": reason,
            ...
            for sample in signal.get("items", signal.get("samples", [])):
                ...
                if sample.get("table_index") is not None and sample.get("cell_index") is not None:
                    source = {
                        ...
                        "cell_index": sample.get("cell_index"),
                        "page": sample.get("page"),
                        "row_start": sample.get("row_start"),
                        "row_end": sample.get("row_end"),
                        "col_start": sample.get("col_start"),
                        "col_end": sample.get("col_end"),
                    }
                else:
            ...
            if previous_ledger_path.is_file():
                try:
                    ...
                    for old_entry in previous_payload.get("entries") or []:
                        item = dict(old_entry)
                        if item.get("status") != "superseded" and not item.get("human_verified"):
