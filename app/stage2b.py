...
from .pipeline_state import verification_signature
...
class Stage2BWorker:
    ...
    async def start_stage2c_backfill(self, postprocess_job_id: int) -> dict[str, Any]:
        ...
        unfinished = [row for row in verification_rows if row.get("status") in {"pending", "processing", "failed"}]
        if unfinished:
        ...
        state = {
            ...
            "verification_signature": verification_signature(verification_rows),
    ...
    async def _device_loop(self, target: str) -> None:
        while not self._stopping.is_set():
            try:
                ...
                if self._artifact_worker_available(target):
