...
def _scope_target_transcription(
    ...
    *,
    ...
    # Guard against scope inversion: trimming must never discard a region that
    # aligns substantially better with the immutable target than the text we
    # kept.  This catches cases where a model returned TARGET followed by a
    # neighboring warning/row and the window selector accidentally retained
    # the neighbor.  We reject instead of silently swapping to the discarded
    # text because source-image localization remains the authority.
    discarded_target_metrics: dict[str, dict[str, float]] = {}
    inversion_detected = False
    for label, removed_text in (("prefix", removed_prefix), ("suffix", removed_suffix)):
        if not removed_text.strip():
            continue
        removed_lines = [(line, idx) for idx, line in enumerate(removed_text.splitlines(), start=1) if line.strip()]
        if not removed_lines:
            continue
        rscore, rseq, rrecall, rprecision, rlength = window_metrics(removed_lines)
        discarded_target_metrics[label] = {
            "scope_score": round(rscore, 4),
            "sequence_similarity": round(rseq, 4),
            "target_token_recall": round(rrecall, 4),
            "target_token_precision": round(rprecision, 4),
            "length_similarity": round(rlength, 4),
        }
        if (
            (rseq >= seq + 0.18 and rrecall >= token_recall + 0.18 and rseq >= 0.45)
            or (rseq >= 0.60 and rrecall >= 0.55 and seq < 0.40)
        ):
            inversion_detected = True

    reasons: list[str] = []
    if inversion_detected:
        reasons.append("SCOPE_INVERSION_DISCARDED_TARGET")
    ...
    alignment_safety = source_transcription_safety_profile(original_text, scoped, source_type=source_type)
    for reason in alignment_safety.get("reasons") or []:
        if reason not in reasons:
            reasons.append(str(reason))

    # Table cells are deliberately cropped tightly.  Do not auto-trim a model
    # response that contains extra non-anchor rows/cells: even if the target
    # line can be isolated lexically, flattening neighboring table content is a
    # higher-risk failure than preserving the immutable cell for review.
    if str(source_type or "text") == "table_cell":
        if removed_prefix.strip() or removed_suffix.strip():
            if "TABLE_CELL_CONTEXT_CONTAMINATION" not in reasons:
                reasons.append("TABLE_CELL_CONTEXT_CONTAMINATION")
        elif (
            token_recall >= 0.70
            and token_precision < 0.65
            and length_similarity < 0.72
            and len(scoped_norm) > max(len(orig_norm) + 20, int(len(orig_norm) * 1.35))
        ):
            reasons.append("TABLE_CELL_CONTEXT_CONTAMINATION")
    ...
    base = {
        ...
        "discarded_target_metrics": discarded_target_metrics,
        "scope_inversion_detected": inversion_detected,
    }
    if reasons:
        base.update({
            "accepted": False,
...
async def _oneplus_text_crosscheck(
    ...
    *,
    ...
    def scope_candidate(candidate: str) -> dict[str, Any]:
        return _scope_target_transcription(candidate, original=original_for_scope, before_anchors=before, after_anchors=after, source_type=source_type)
...
class Stage2BWorker:
    ...
    def device_lock(self, provider: str) -> asyncio.Lock:
        ...
        This keeps a manual answer request from colliding with Stage 2B on the
    ...
    async def start(self) -> None:
        ...
        if recovered_outages:
            ...
            self._events.notify("stage2b_endpoint_outage_jobs_requeued")
    ...
    def _auto_run(self, target: str) -> bool:
        ...
        return bool(
            getattr(config, "stage2b_pi5_auto_run", False)
            if target == "pi5" else getattr(config, "stage2b_oneplus_auto_run", False)
        )
    ...
    async def _persist_manual_crosscheck(self, job: dict[str, Any], payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            source = {}
        is_picture = str(source.get("type") or "") == "picture"
    ...
    async def _run_manual_crosscheck(self, job: dict[str, Any]) -> None:
        ...
        try:
            ...
            is_picture = source_type == "picture"
            state["started_at_epoch"] = time.time()
            if not is_picture and job.get("target") == "pi5":
                ...
                client = self._vision_client_for_role(opposite, job)
                ...
                client = self._client_for(opposite, job)
                endpoint = str(getattr(client, "endpoint", self._endpoint_for_provider(str(getattr(client, "provider", opposite)))))
                model = await self._model_for(f"manual-text:{opposite}", endpoint, client)
                state["status"] = "running"
        ...
        for row in rows:
            ...
            if direct:
                ...
                if str(correction.get("status") or "") == "applied" and proposed:
                    profile = source_transcription_safety_profile(original, proposed, source_type=str(request.get("source_type") or (request.get("source") or {}).get("type") or "text"))
    ...
    async def start_stage2c_backfill(self, postprocess_job_id: int) -> dict[str, Any]:
        ...
        unfinished = [row for row in effective_rows if row.get("status") in {"pending", "processing", "failed"}]
        if unfinished:
            failed = sum(1 for row in unfinished if row.get("status") == "failed")
            waiting = sum(1 for row in unfinished if row.get("status") in {"pending", "processing"})
            details = []
            if waiting:
                details.append(f"{waiting} pending/processing")
            if failed:
                details.append(f"{failed} failed")
            raise ValueError("Stage 2B must finish successfully before Stage 2C can run (" + ", ".join(details) + ")")
        completed = [row for row in effective_rows if row.get("status") == "completed"]
        optional_sweep_only = bool(verification_rows) and not effective_rows and not bool(
            getattr(config, "stage2b_artifact_sweep_required_for_finalize", True)
        )
        if not completed and not optional_sweep_only and not allow_empty:
        ...
        state = {
            "postprocess_job_id": postprocess_job_id,
            "status": "queued",
            "total_verification_routes": len(effective_rows),
            "all_verification_routes": len(verification_rows),
            "completed_verification_routes": len(completed),
            "failed_verification_routes": 0,
            "artifact_sweep_required": bool(getattr(config, "stage2b_artifact_sweep_required_for_finalize", True)),
            "verification_signature": verification_signature(effective_rows),
        ...
        try:
            ...
            for row in rows:
                ...
                try:
                    ...
                    try:
                        row_source = json.loads(row.get("source_json") or "{}")
                    except json.JSONDecodeError:
                        row_source = {}
                    is_picture_row = str(row_source.get("type") or "") == "picture"
                    if target == "pi5" and not is_picture_row:
                        original = str(request.get("suspect_text") or "")
                        context = str(request.get("nearby_context") or "")
                    ...
                    elif target == "oneplus" or is_picture_row:
                        old_parsed = stored_result.get("parsed") or {}
    ...
    async def _prepare_artifact_sweep_for_book(
        self, job: dict[str, Any], generation: str, result_dir: Path, manifest: dict[str, Any]
    ) -> dict[str, Any]:
        """Prepare a book's technical-artifact sweep without authorizing it."""
        if not bool(getattr(self._config_getter(), "stage2b_artifact_sweep_enabled", True)):
            return {"created": 0, "technical_visuals": 0, "skipped_normal_picture_routes": 0, "disabled": True}
        converted_name = Path(str(manifest.get("converted_zip") or job.get("output_filename") or "")).name
        output_dir = str(getattr(self._config_getter(), "output_dir", "") or "")
        if not output_dir:
            return {"created": 0, "technical_visuals": 0, "skipped_normal_picture_routes": 0}
        zip_path = Path(output_dir) / converted_name
        if not converted_name or not zip_path.is_file():
            return {"created": 0, "technical_visuals": 0, "skipped_normal_picture_routes": 0}
        existing_rows = await self._store.list_book_jobs_raw(int(job["id"]))
        try:
            plan = await asyncio.to_thread(build_artifact_sweep_plan, zip_path, existing_rows)
        except (OSError, ValueError, zipfile.BadZipFile):
            logger.exception("Could not prepare artifact sweep for book %s", job.get("id"))
            return {"created": 0, "technical_visuals": 0, "skipped_normal_picture_routes": 0}
        ...
        return {
            "created": int(created),
            "technical_visuals": int(plan.get("technical_visuals") or 0),
            ...
            for job in jobs:
                ...
                if manifest_path.is_file():
                    try:
                        ...
                        manifest_sha = str(manifest.get("converted_zip_sha256") or "")
                    except (OSError, json.JSONDecodeError, TypeError):
                        manifest = {}
                        manifest_sha = ""
                ...
                sweep = await self._prepare_artifact_sweep_for_book(job, generation, result_dir, manifest)
                created_total += int(sweep.get("created") or 0)
                # A book with no normal Text/Vision routes has already satisfied
                # the dependency gate, so its prepared sweep can start immediately.
                # No extra click or Auto Run combination is required.
                if (not routes and bool(getattr(self._config_getter(), "stage2b_artifact_sweep_enabled", True))
                        and hasattr(self._store, "arm_artifact_sweep_for_book")):
                    await self._store.arm_artifact_sweep_for_book(postprocess_job_id)
                    await self._store.release_ready_artifact_sweeps(postprocess_job_id)
    ...
    def _is_artifact_sweep_job(job: dict[str, Any]) -> bool:
        if str(job.get("code") or "") == "FULL_TECHNICAL_VISUAL":
            return True
        try:
            source = json.loads(job.get("source_json") or "{}")
        except (json.JSONDecodeError, TypeError):
    ...
    def _cooldown_artifact_worker(self, target: str) -> None:
        ...
        self.worker_state[target]["artifact_cooldown_until_epoch"] = until
    ...
    def _endpoint_outage_payload(self, provider: str, detail: str) -> dict[str, Any]:
        ...
        return {
            "schema": "docling-stage2b-endpoint-outage/v1",
            "provider": provider,
            "endpoint": self._endpoint_for_provider(provider),
            "opened_at_epoch": state.get("opened_at_epoch"),
            "next_probe_at_epoch": state.get("next_probe_at_epoch"),
            "failure_count": state.get("failure_count"),
            "last_error": detail[:2000],
            "recovered_at_epoch": state.get("recovered_at_epoch"),
            "note": "Endpoint outage is infrastructure state. Job retry_count is not consumed.",
        }

    def _write_endpoint_outage_file_sync(self, provider: str, detail: str) -> str:
        state = self._endpoint_circuit[provider]
        outage_dir = Path(self._config_getter().processed_dir) / "_verification_outages"
        outage_dir.mkdir(parents=True, exist_ok=True)
        existing = state.get("outage_path")
        if existing:
    ...
    async def _open_endpoint_circuit(
        ...
        base = max(5, int(getattr(self._config_getter(), "stage2b_endpoint_breaker_base_seconds", 30)))
        maximum = max(base, int(getattr(self._config_getter(), "stage2b_endpoint_breaker_max_seconds", 300)))
        delay = min(maximum, base * (2 ** min(max(0, state["failure_count"] - 1), 6)))
        state["next_probe_at_epoch"] = now + delay
        state["last_error"] = str(detail)[:2000]
    ...
    async def _close_endpoint_circuit(self, provider: str) -> None:
        ...
        if not state or not state.get("open"):
            return
        state["open"] = False
        state["recovered_at_epoch"] = time.time()
        state["next_probe_at_epoch"] = 0.0
        state["last_error"] = None
        self._sync_endpoint_circuit_state(provider)
        try:
            await asyncio.to_thread(self._write_endpoint_outage_file_sync, provider, "endpoint recovered")
        except OSError:
    ...
    async def _endpoint_provider_ready(self, provider: str) -> bool:
        ...
        if time.time() < float(state.get("next_probe_at_epoch") or 0.0):
            return False
        async with self._endpoint_probe_locks[provider]:
            state = self._endpoint_circuit[provider]
            if not state.get("open"):
                return True
            if time.time() < float(state.get("next_probe_at_epoch") or 0.0):
    ...
    async def _maybe_auto_finalize_book(self, postprocess_job_id: int) -> None:
        ...
        signature_rows = verification_rows_for_stage2c(
            rows,
            artifact_sweep_required=bool(getattr(config, "stage2b_artifact_sweep_required_for_finalize", True)),
        )
        allow_empty = not bool(getattr(config, "stage2b_artifact_sweep_enabled", True))
        ...
        *,
        ...
        try:
            ...
            completed_provider = str(
                (job.get("_artifact_worker") if is_artifact_sweep else (request.get("vision_provider") or request.get("provider") or self._selected_provider(target)))
                or target
            ).lower()
    ...
    async def _record_stage2c_entry(
        ...
        target_crop = request.get("target_crop") if isinstance(request.get("target_crop"), dict) else {}
        common = {
            ...
            "row_start": source.get("row_start") if source.get("row_start") is not None else target_crop.get("row_start"),
            "row_end": source.get("row_end") if source.get("row_end") is not None else target_crop.get("row_end"),
            "col_start": source.get("col_start") if source.get("col_start") is not None else target_crop.get("col_start"),
            "col_end": source.get("col_end") if source.get("col_end") is not None else target_crop.get("col_end"),
            "model": model,
            "verification_provider": (request.get("vision_provider") or request.get("provider") or target),
            "verification_worker": target,
            "rule_version": STAGE2C_RULE_VERSION,
            "created_at_epoch": time.time(),
            "verification_verdict": verdict,
            "artifact_sweep": bool(source.get("artifact_sweep")),
        }
        if target == "pi5" and str(source.get("type") or "") != "picture":
            parsed = result.get("parsed") or {}
    ...
    def _vision_client_for_role(self, role_target: str, job: dict[str, Any]) -> CheckpointVerifier:
        ...
        try:
            ...
            source = {}
        # Normal verification follows the explicitly selected role provider.
        # FULL_TECHNICAL_VISUAL is intentionally different: it is a large local
        # batch and uses a shared Pi5/OnePlus work-stealing pool. The worker that
    ...
    async def _run_pi5(self, job: dict[str, Any]):
        ...
        request = {
            ...
            "table_index": source.get("table_index"),
            "cell_index": source.get("cell_index"),
            "row_start": source.get("row_start") if source.get("row_start") is not None else (crop_meta or {}).get("row_start"),
            "row_end": source.get("row_end") if source.get("row_end") is not None else (crop_meta or {}).get("row_end"),
            "col_start": source.get("col_start") if source.get("col_start") is not None else (crop_meta or {}).get("col_start"),
            "col_end": source.get("col_end") if source.get("col_end") is not None else (crop_meta or {}).get("col_end"),
            "source_type": source_type,
            "suspect_text": suspect,
    ...
    async def _run_oneplus(self, job: dict[str, Any], role_target: str = "oneplus"):
        config = self._config_getter()
        zip_path = Path(config.output_dir) / str(job["output_filename"])
        doc = await self._document_for(zip_path)
        source = json.loads(job["source_json"] or "{}")
    ...
    async def text_audit_image(self, job_id: int) -> tuple[bytes, str, str]:
        ...
        Prefer the clip/pixel coordinates saved in request_json so configuration
        changes after the original run cannot silently alter audit evidence. The
        immutable source PDF/raster is read only; no verification is rerun.
        """
        job = await self._store.get_job(int(job_id))
        if not job or str(job.get("target") or "") != "pi5":
            raise ValueError("Text verification job not found")
        try:
            request = json.loads(job.get("request_json") or "{}")
        except json.JSONDecodeError:
            request = {}
        try:
            source_ref = json.loads(job.get("source_json") or "{}")
        except json.JSONDecodeError:
            source_ref = {}
        ...
        conversion = await self._postprocess_store.get_conversion_job(int(job.get("conversion_job_id") or 0))
        source_filename = str((conversion or {}).get("filename") or (conversion or {}).get("source_filename") or "")
        source_path = Path(config.input_dir) / Path(source_filename).name
        crop = request.get("target_crop") if isinstance(request, dict) else None
        crop = crop if isinstance(crop, dict) else {}
        page_value = request.get("page") if isinstance(request, dict) else None
        if page_value is None:
            page_value = source_ref.get("page")
        page = int(page_value) if page_value is not None else None

        if source_path.is_file() and page is not None and crop:
            source_kind = str(crop.get("source_kind") or "")
            if source_kind == "pdf" and source_path.suffix.lower() == ".pdf":
                points = crop.get("clip_points")
                if isinstance(points, list) and len(points) == 4:
                    with fitz.open(source_path) as pdf:
                        page_index = page - 1
                        if 0 <= page_index < len(pdf):
                            clip = fitz.Rect(*(float(v) for v in points))
                            scale = float(crop.get("scale") or 1.0)
                            pix = pdf[page_index].get_pixmap(matrix=fitz.Matrix(scale, scale), clip=clip, alpha=False)
                            return pix.tobytes("png"), "image/png", f"page {page} · {crop.get('mode') or 'saved crop'}"
            if source_kind == "raster_image" and source_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp", ".tif", ".tiff", ".bmp"}:
                px = crop.get("pixel_box")
                if isinstance(px, list) and len(px) == 4:
                    with Image.open(source_path) as opened:
                        try:
                            opened.seek(max(0, page - 1))
                        except EOFError as exc:
                            raise ValueError("Saved text-audit image page is unavailable") from exc
                        image = opened.convert("RGB")
                        ...
                        if scale > 1.0:
                            target = target.resize(
                                (max(1, int(target.width * scale)), max(1, int(target.height * scale))),
                                Image.Resampling.LANCZOS,
                            )
                        buf = io.BytesIO()
                        target.save(buf, format="PNG", optimize=True)
                        return buf.getvalue(), "image/png", f"page {page} · {crop.get('mode') or 'saved crop'}"

        # Compatibility fallback for older results that predate persisted crop
        # geometry. It still uses the immutable original source and Docling
        # locator, but the label makes clear that it was reconstructed today.
        zip_path = Path(config.output_dir) / str(job.get("output_filename") or "")
        doc = await self._document_for(zip_path)
        source_type = str(request.get("source_type") or source_ref.get("type") or "text")
        if source_type == "table_cell":
            table_index = int(request.get("table_index") if request.get("table_index") is not None else source_ref.get("table_index"))
            cell_index = int(request.get("cell_index") if request.get("cell_index") is not None else source_ref.get("cell_index"))
            rendered = await asyncio.to_thread(
                _render_source_table_cell, config, source_filename, page, doc, table_index, cell_index
            )
        else:
            index = int(request.get("text_index") if request.get("text_index") is not None else source_ref.get("index"))
            rendered = await asyncio.to_thread(_render_source_target, config, source_filename, page, doc, index)
        if not rendered:
            raise FileNotFoundError("Original text target crop is unavailable")
        image_bytes, mime, meta = rendered
        return image_bytes, mime, f"page {page or '—'} · {meta.get('mode') or 'reconstructed crop'}"
    ...
    async def vision_audit_image(self, job_id: int, region: str = "full") -> tuple[bytes, str, str]:
        ...
        """
        job = await self._store.get_job(int(job_id))
        if not job or str(job.get("target") or "") not in {"pi5", "oneplus"}:
            raise ValueError("Vision verification job not found")
        try:
            audit_source = json.loads(job.get("source_json") or "{}")
        except json.JSONDecodeError:
            audit_source = {}
        if str(audit_source.get("type") or "") != "picture":
            raise ValueError("Vision verification job not found")
        config = self._config_getter()
        zip_path = Path(config.output_dir) / str(job.get("output_filename") or "")
        doc = await self._document_for(zip_path)
