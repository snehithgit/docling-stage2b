...
class AppConfig:
    ...
    # Open a per-provider circuit breaker, preserve the current job's retry budget,
    ...
    stage2b_artifact_worker_cooldown_seconds: int = 60
    # Full technical visual sweep orchestration. When enabled, Stage 2A prepares
    # sweep rows automatically and they run only after normal Text/Vision routes.
    # Set required_for_finalize=false when a fast RAG-ready path is preferred;
    # completed sweep evidence will still make Stage 2C stale and rebuildable.
    stage2b_artifact_sweep_enabled: bool = True
    stage2b_artifact_sweep_required_for_finalize: bool = True
    ...
    # Retrieval answer generation. Search remains deterministic and model-free;
    # an LLM is called only when the user explicitly presses Generate answer.
    # The same retrieved chunks can also be exported as a portable prompt with
    # zero model calls. No automatic provider fallback is allowed.
    rag_answer_max_sources: int = 5
    rag_answer_local_max_tokens: int = 640
    rag_answer_cloud_max_tokens: int = 900
    rag_answer_local_timeout_seconds: int = 900
    ...
    retrieval_embedding_document_prefix: str = ""
    retrieval_embedding_batch_size: int = 32
    retrieval_embedding_timeout_seconds: int = 180
    retrieval_hybrid_candidate_depth: int = 60
    ...
    def validate(self) -> None:
        ...
        if self.stage2b_endpoint_breaker_base_seconds < 5:
        ...
        if not 1 <= int(self.rag_answer_max_sources) <= 10:
            raise ValueError("RAG answer max sources must be between 1 and 10")
        if self.rag_answer_local_max_tokens < 64:
            raise ValueError("RAG local answer max tokens must be at least 64")
        if self.rag_answer_cloud_max_tokens < 64:
            raise ValueError("RAG cloud answer max tokens must be at least 64")
        if self.rag_answer_local_timeout_seconds < 30:
            raise ValueError("RAG local answer timeout must be at least 30 seconds")
        if not self.retrieval_embedding_url.startswith(("http://", "https://")):
            raise ValueError("Retrieval embedding URL must begin with http:// or https://")
        if not self.retrieval_embedding_model.strip():
            raise ValueError("Retrieval embedding model cannot be empty")
        if not 1 <= int(self.retrieval_embedding_batch_size) <= 128:
            raise ValueError("Retrieval embedding batch size must be between 1 and 128")
        if int(self.retrieval_embedding_timeout_seconds) < 10:
            raise ValueError("Retrieval embedding timeout must be at least 10 seconds")
        if not 10 <= int(self.retrieval_hybrid_candidate_depth) <= 500:
    ...
    def public_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            "telegram_bot_token_configured": bool(os.environ.get(self.telegram_bot_token_env, "").strip()),
            "telegram_allowed_chat_ids": list(self.telegram_allowed_chat_ids),
            ...
            "rag_answer_max_sources": self.rag_answer_max_sources,
            ...
            "retrieval_hybrid_rrf_k": self.retrieval_hybrid_rrf_k,
        }
