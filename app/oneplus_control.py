...
class OnePlusController:
    ...
    async def _status_unlocked(self) -> dict[str, Any]:
        ...
        try:
            command = (
                f'printf "SSH_OK\\n"; '
                f'if [ -x {script_expr} ]; then '
                f'printf "SCRIPT_READY=1\\n"; '
                f'status=$({script_expr} status 2>/dev/null || true); '
                f'printf "LLAMA_STATUS=%s\\n" "$status"; '
                f'else printf "SCRIPT_READY=0\\n"; fi'
            )
            result = await self._run_ssh(command, timeout=7.0)
            ready = "SCRIPT_READY=1" in result.stdout
            llama_status = None
            for line in result.stdout.splitlines():
                if line.startswith("LLAMA_STATUS="):
                    llama_status = line.split("=", 1)[1].strip() or None
                    break
            running = bool(llama_status and llama_status.startswith("RUNNING"))
            return {
                "ssh": {"reachable": True, "error": None},
                "script_ready": ready,
                "llama": {"running": running, "status": llama_status or ("Unknown" if ready else "Script not installed")},
            ...
            return {
                ...
                "script_ready": False,
                "llama": {"running": False, "status": "Unavailable"},
                "password_configured": self.password_configured(),
                "config": self.public_config(),
                "last_action": self._last_action,
            }
