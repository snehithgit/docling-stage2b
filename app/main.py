...
from .stage2c import STAGE2C_RULE_VERSION, apply_human_correction_to_entry, human_review_summary, upsert_ledger_entry
        ...
        for job in jobs:
            ...
            if total:
                ...
                if not blockers:
                    ...
                    stage2c = stage2c_freshness(result_dir, rows, rule_version=STAGE2C_RULE_VERSION)
...
async def documents() -> dict:
    ...
    for row in rows:
        ...
        if row.get("status") == "completed" and row.get("result_dir"):
            ...
            s2c = stage2c_freshness(result_dir, verification_rows, rule_version=STAGE2C_RULE_VERSION) if verification_rows else {"ready": False, "reason": "stage2b_not_started"}
...
async def _retrieval_books() -> list[dict]:
    ...
    for job in jobs:
        ...
        stage2c_info = stage2c_freshness(result_dir, verification_rows, rule_version=STAGE2C_RULE_VERSION) if verification_rows else {"ready": False, "reason": "stage2b_not_started"}
...
async def _prepare_full_artifact_sweep(postprocess_job_id: int | None = None) -> dict:
    ...
    for job in selected:
        ...
        zip_path = Path(runtime.config.output_dir) / converted_name
        if not converted_name or not zip_path.is_file():
            books.append({"postprocess_job_id": int(job.get("id") or 0), "book": job.get("source_filename") or job.get("output_filename"), "status": "missing_converted_zip", "created": 0})
            continue
        try:
            pictures = await asyncio.to_thread(_docling_pictures, zip_path)
        except (OSError, ValueError, zipfile.BadZipFile) as exc:
            books.append({"postprocess_job_id": int(job.get("id") or 0), "book": job.get("source_filename") or job.get("output_filename"), "status": f"error: {exc}", "created": 0})
            continue
        ...
        for picture_index, picture in enumerate(pictures):
            class_name, confidence = _picture_top_class(picture)
            if not _is_technical_picture_class(class_name):
                continue
            book_technical += 1
            technical_total += 1
            prov = picture.get("prov") or []
            ...
            image = picture.get("image") or {}
            # FULL_TECHNICAL_VISUAL uses one shared local artifact pool. Keep
            # the historical ``oneplus`` DB target for upgrade compatibility;
            # execution is claimed dynamically by whichever idle local worker
            # (Pi5 or OnePlus) is available. No fixed split is assigned here.
            target = "oneplus"
            per_worker["shared_pool"] = int(per_worker.get("shared_pool") or 0) + 1
            sweep_jobs.append({
        ...
        created = await runtime.stage2b_store.create_artifact_sweep_jobs(
            int(job.get("id") or 0),
            int(job.get("conversion_job_id") or 0),
            generation,
            Path(str(job.get("result_dir") or "")).name,
            converted_name,
            sweep_jobs,
        )
        created_total += int(created)
        books.append({"postprocess_job_id": int(job.get("id") or 0), "book": job.get("source_filename") or job.get("output_filename"), "technical_visuals": book_technical, "created": int(created), "status": "ready"})
    return {"technical_visuals": technical_total, "created": created_total, "assigned": per_worker, "books": books}
...
async def stage2b_artifact_audit_start_all(postprocess_job_id: int | None = None) -> dict:
    ...
    # Authorize both historical DB lanes, then let the two local workers steal
    # from one shared artifact pool. Manual Pause is respected: starting a sweep
    # never silently unpauses a hot/offline device.
    authorized_legacy_pi5 = await runtime.stage2b_store.start_artifact_sweep("pi5")
    authorized_vision = await runtime.stage2b_store.start_artifact_sweep("oneplus")
    if authorized_legacy_pi5 or authorized_vision:
        runtime.events.notify("stage2b_artifact_audit_start_all")
    return {
        "accepted": bool(authorized_legacy_pi5 or authorized_vision or prepared.get("created")),
        "prepared": prepared,
        "authorized": {
            "total": int(authorized_vision + authorized_legacy_pi5),
            "historical_oneplus_lane": int(authorized_vision),
            "historical_pi5_lane": int(authorized_legacy_pi5),
        },
        "artifact_workers": {
...
async def stage2b_artifact_audit_retry_failed() -> dict:
    ...
    # Keep manual Pause authoritative. A failed artifact row is returned to the
    # shared pool, but starting/retrying a batch never forces a paused device on.
    if retried_pi5 or retried_oneplus:
